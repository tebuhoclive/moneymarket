...
export const ClientInformationForm = (props: IClientInformationFormProps) => {
    ...
    const handleUpDraft = async () => {
        setLoading(true);
        const draft: INaturalPerson = {
            ...client,
            entityDisplayName: `${client.clientSurname} ${client.clientName}`,
            profileStatus: "Draft Pending Submission",
            lastUpdated: Date.now(),
            entityType: "individual",
        }
        try {
            await api.client.naturalPerson.update(draft);
            swal({
                icon: "success",
                title: "Client profile draft updated"
            });
            onCancel();
        } catch (error) {
        }
    ...
    const handleSaveDraft = async () => {
        ...
        const draft: INaturalPerson = {
            ...
            entityDisplayName: `${client.clientSurname} ${client.clientName}`,
            profileStatus: "Draft Pending Submission",
            timeDrafted: Date.now(),
            lastUpdated: Date.now(),
            entityType: "Individual",

        }
        try {
            await api.client.naturalPerson.create(draft);
            swal({
                icon: "success",
                title: "Client profile draft saved"
            });
            onCancel();
        } catch (error) {
        }
            ...
            <form className="uk-form-stacked uk-grid uk-grid-small uk-padding" onSubmit={onSubmitClientInformation} data-uk-grid>
                ...
                <div className="uk-width-1-1 uk-text-right">
                    ...
                    </button>
                    {
                        ...
                        <button onClick={handleUpDraft} className="btn btn-warning" type="button" disabled={loading}>
                            Update Draft
                    ...
                    {/* {
                        !store.client.naturalPerson.selected &&
                        <button onClick={handleSaveDraft} className="btn btn-warning" type="button" disabled={loading}>
                            Save As Draft
                            {loading && <span data-uk-spinner="ratio: .5"></span>}
                        </button>
                    } */}
