...
const BankStatementUploadModal = observer(({ isVisible }: IProps) => {
  ...
  const [initialSuggestedMatch, setInitialSuggestedMatch] = useState<ISuggestedMatchedTransaction[]>([]);
  ...
  const [initialUnmatched, setInitialUnmatched] = useState<IUnMatchedTransaction[]>([]);
...
const handleRefreshIndexed = (transactions: IIndexedTransaction[]) => {
  ...
  // Section 1: Handle "Used Client Reference" transactions (IMatchedTransaction)
  const clientReferenceTransactions = transactions.filter(
    (transaction) => transaction.matchType === "Client"
  );

      // Create a map to track updated transactions by statementIdentifier
      const transactionMap = new Map<string, IIndexedTransaction>(
        clientReferenceTransactions.map((transaction) => [
            transaction.statementIdentifier,
            {
                ...transaction,
                matchType: "Client",
    ...
    setSelectedIndexedTransactions(filteredTransactions)
    console.log("Final transactions:", newTransactions);
  console.log("Current selected indexed transactions:", selectedIndexedTransactions);




  // Section 2: Handle "System Accepted Match" transactions (ISuggestedMatchedTransaction)
  const suggestedTransactions = transactions.filter(
    (transaction) => transaction.matchType === "System Suggested"
  );
      // Create a map to track updated transactions by statementIdentifier
      const suggestedTransactionMap = new Map<string, IIndexedTransaction>(
        suggestedTransactions.map((transaction) => [
            transaction.statementIdentifier,
            {
                ...transaction,
                
                isIndexed: false, // Mark as not indexed
             matchType: "System Accepted Match",
        suggestionRemark: "Accepted"
    ...
    // Generate newTransactions array
    const newSuggestedTransactions: IIndexedTransaction[] = Array.from(suggestedTransactionMap.values());
    // Update matched items based on the same identifier only if they exist in transactions
    const newSuggestedItems: ISuggestedMatchedTransaction[] = initialSuggestedMatch.map((item) => {
        const updatedTransaction = transactionMap.get(item.statementIdentifier);
        // Update only if the item exists in the transactionMap and needs to be updated
        if (updatedTransaction) {
            return {
    ...
    // Filter transactions where isIndexed is true
    const filteredTransactionsSuggestion = newSuggestedTransactions.filter(t => t.isIndexed ===true);
    const newSuItems = newMatchedItems.filter(t => t.isIndexed === false);
   
    // Set the filtered transactions
    setSuggestedMatch(newSuggestedItems)
    setSelectedSuggestedTransactions(newSuggestedItems)
    setIndexTransactions(filteredTransactionsSuggestion);
      // Concatenate the filtered transactions with the existing indexed transactions
  //     setSelectedIndexedTransactions((prevTransactions) => [
  //   ...prevTransactions, // Keep existing transactions
  //   ...filteredTransactionsSuggestion // Add the new filtered transactions
  // ]);
    setSelectedIndexedTransactions(filteredTransactionsSuggestion)
    console.log("Final transactions:", newTransactions);


  ////////////////////////////////////////end 
  
   // Section 3: Handle "Umatched" transactions (IUmatchedTransaction)
   const clientUnmatchedTransactions = transactions.filter(
    (transaction) => transaction.matchType === "System Rejected Match"
  );

    // Section 2: Handle "System Accepted Match" transactions (ISuggestedMatchedTransaction)
  
        // Create a map to track updated transactions by statementIdentifier
        const unmatchedTransactionMap = new Map<string, IIndexedTransaction>(
          clientUnmatchedTransactions.map((transaction) => [
              transaction.statementIdentifier,
              {
                  ...transaction,
      ...
      // Generate newTransactions array
      const newUnmatchedTransactions: IIndexedTransaction[] = Array.from(unmatchedTransactionMap.values());
      // Update matched items based on the same identifier only if they exist in transactions
      const newUmatchedItems: IUnMatchedTransaction[] = initialUnmatched.map((item) => {
          const updatedTransaction = transactionMap.get(item.statementIdentifier);
          // Update only if the item exists in the transactionMap and needs to be updated
          if (updatedTransaction) {
              return {
      ...
      // Filter transactions where isIndexed is true
      const filteredTransactionsUnmatched = newUnmatchedTransactions.filter(t => t.isIndexed ===true);
      const newSuItemsee = newMatchedItems.filter(t => t.isIndexed === false);
     
      // Set the filtered transactions
      setUnmatched(newUmatchedItems)
      setSelectedUnmatchedTransactions(newUmatchedItems)
      setIndexTransactions(filteredTransactionsUnmatched);
        // Concatenate the filtered transactions with the existing indexed transactions
    //     setSelectedIndexedTransactions((prevTransactions) => [
    //   ...prevTransactions, // Keep existing transactions
    //   ...filteredTransactionsSuggestion // Add the new filtered transactions
    // ]);
      setSelectedIndexedTransactions(filteredTransactionsUnmatched)
      console.log("Final transactions:", newTransactions);
  ...
  const handleUpload = async () => {
    ...
    setInitialSuggestedMatch(suggestedItems)
    setMatched(finalMatchedItems);
    setUnmatched(unmatchedItems);
    setInitialUnmatched(unmatchedItems)
        ...
        <div className="uk-text-right">
          {
            ...
            <>
              ...
              {
                accountNumber.trim() === config.accountNumber &&tab === "Indexed Transactions" &&
