...
import { completeTransaction } from "./complete-transaction-process/ProcessTransactions";
import { CompleteBackDate } from "./complete-transaction-process/BackDateTransaction";
import { getMMADocId } from "../../../../../../shared/functions/MyFunctions";

const SecondLevelTransactionQueueView = observer(() => {
    ...
    const transaction: IDepositTransaction = {
        ...
        valueDate: depositTransaction.valueDate,
        amount: depositTransaction.amount,
        accountNumber: depositTransaction.accountNumber,
        productCode: depositTransaction.productCode,
        entityNumber: depositTransaction.entityNumber,
        sourceBank: depositTransaction.sourceBank,
        secondLevelApprover: store.auth.meUID || "",
        bankReference: depositTransaction.bankReference,
        sourceOfFundsAttachment: {
            url: depositTransaction.sourceOfFundsAttachment.url,
            reasonForNotAttaching: depositTransaction.sourceOfFundsAttachment.reasonForNotAttaching
        },
        proofOfPaymentAttachment: {
            url: depositTransaction.proofOfPaymentAttachment.url,
            reasonForNotAttaching: depositTransaction.proofOfPaymentAttachment.reasonForNotAttaching
        },
        sourceOfFunds: depositTransaction.sourceOfFunds,
        depositNodeType: depositTransaction.depositNodeType,
        statementIdentifier: depositTransaction.statementIdentifier,
        createdAtTime: {
            completedQueue: Date.now(),
        },
        transactionType: depositTransaction.transactionType,
        allocationStatus: depositTransaction.allocationStatus,
    ...
    const onCompleteTransaction = async () => {
        const { depositTransactionProcess } = depositTransaction;
        const successMessage = {
            icon: "success",
            title: "Transaction Success",
            text: "System processed transaction successfully"
        };
        const errorMessage = {
            icon: "warning",
            title: "Oops",
            text: "System having trouble completing transaction. please contact the IT team"
        };

        try {

            switch (depositTransactionProcess) {
                case "Normal":
                case "Future-Dated":
                    await completeTransaction(transaction, store, api, setLoading);
                    swal(successMessage);
                    break;

                case "Back-Dated":
                    await CompleteBackDate(transaction, store, api, setLoading);
                    swal(successMessage);
                    break;
        ...
        <ErrorBoundary>
            <div className="custom-modal-style uk-modal-dialog uk-width-4-5">
                ...
                <div className="uk-text-right">
                    ...
                    <button className="btn btn-primary" disabled={loading} onClick={onCompleteTransaction}> {loading ? <span data-uk-spinner={"ratio:.5"}> </span> : "Complete Transaction"}</button>
                </div>
            </div >
        </ErrorBoundary >
    );
...
export default SecondLevelTransactionQueueView;