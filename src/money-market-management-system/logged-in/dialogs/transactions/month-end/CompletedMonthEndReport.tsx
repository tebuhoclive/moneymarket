...
import { CompletedReportMonthEndGrid } from "../../../../../shared/functions/transactions/month-end-report-grid/rolled-back/CompletedMonthEndReportGrid";
...
export const CompletedMonthEndReport = observer(({ setIsVisible }: IProps) => {
    const { api, store } = useAppContext();
    const [selectedMonthEndRun, setSelectedMonthEndRun] = useState<IMonthEndRun>({ ...defaultMonthEndRun })
    const month = new Date(selectedMonthEndRun.date).getMonth();
    const year = new Date(selectedMonthEndRun.date).getFullYear();
    ...
    return (
        <div className="month-end-modal view-modal custom-modal-style uk-modal-dialog uk-modal-body uk-width-1-2" style={{ width: "100%" }}>
            ...
            <h3 className="main-title-sm text-to-break">
                Month End Report ({dateFormat_YY_MM_DD_NEW(selectedMonthEndRun.date)})
            </h3>
            ...
            {isPreparingData ? <LoadingEllipsis /> :
                <div>
                    <CompletedReportMonthEndGrid data={processedData} year={`${year}`} month={`${month}`} />
                </div>
            }
        </div>
    )
