...
const RecordEntitySwitchModalCloseOut = observer(({ setVisible }: IProps) => {
  ...
  const fromAccountDetails: IDataDisplay[] = [
    ...
    },
    {
      ...
      value: currencyFormat(closeOutInterest)
    },
    {
      label: "Capitalised Amount",
      value: currencyFormat(closeOutInterest + (fromAccount?.asJson.balance || 0))
    },
  ]
    ...
    if (selectedAccount) {
      ...
      if (account) {
        ...
        if (client) {
          ...
          const interest = calculateCloseOutInterest(transactions, startOfMonth, endOfMonth, switchTransaction.valueDate, setLoading);
  ...
  const handleSwitchBetweenAccounts = (e: FormEvent<HTMLFormElement>) => {
    ...
    swal({
      ...
      if (edit) {
        ...
        if (fromAccount && toAccount) {
          if (futureDatingTransaction) {
            const _switchTransaction: ISwitchTransaction = {
              ...
              amount: (fromAccount?.asJson.balance || 0) + closeOutInterest,
        ...
        swal({
          icon: "success",
          text: "Switch Close Out Recorded",
        });
        onCancel();
      }
    });
  ...
  const handleDateChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const transactions = store.statementTransaction.all
      .filter((transaction) => transaction.asJson.blinded !== true)
      .map((transaction) => { return transaction.asJson })
    const interest = calculateCloseOutInterest(transactions, startOfMonth, endOfMonth, selectedDate, setLoading);
    setCloseOutInterest(interest.totalDistribution);
    setCloseOutDays(interest.totalDays);
    //close interest
  ...
  const onCancel = () => {
    setSwitchTransaction({ ...defaultSwitchTransaction });
    setSwitchTransactionTo({ ...defaultSwitchTransaction });
    hideModalFromId(MODAL_NAMES.BACK_OFFICE.SWITCH_BETWEEN_ENTITIES_MODAL_CLOSE_OUT);
    setSelectedClient(null);
    setSelectedClientTo(null);
    setSelectedClientAccountNumberTo({ label: "", value: "" })
    setSelectedClientAccountNumber({ label: "", value: "" })
      ...
      <div className="view-modal custom-modal-style uk-modal-dialog uk-modal-body uk-width-4-5">
        ...
        <div className="form-title">
          ...
          <h3 className="text-to-break" style={{ marginLeft: "2rem" }}>
            Manual Switch Transaction Close Out
          </h3>
        </div>
        <hr />
...
export default RecordEntitySwitchModalCloseOut;
