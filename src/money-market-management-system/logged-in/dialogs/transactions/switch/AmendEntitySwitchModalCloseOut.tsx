...
import { calculateCloseOutInterest } from "../withdrawal-transaction/CloseModalNewInterestFunction";
...
import DeleteEntitySwitchCloseOutModal from "./DeleteEntitySwitchCloseOutModal";
...
const AmendEntitySwitchModalCloseOut = observer(({ setVisible }: IProps) => {
        ...
        if (fromAccount && toAccount) {
          if (futureDatingTransaction) {
            const _switchTransaction: ISwitchTransaction = {
              ...
              reference: switchTransaction.reference || `Switch from ${fromAccount?.accountNumber} to ${toAccount.accountNumber}`,
            ...
            try {
              ...
              console.log("Error updating futuredated " + error)
            }
        ...
        if (fromAccount && toAccount) {
          if (futureDatingTransaction) {
            const _switchTransaction: ISwitchTransaction = {
              ...
              switchTransactionProcess: "Future-Dated",
              description: switchTransaction.description,
              amount: fromAccount?.asJson.balance,
              switchStatus: "Draft",
              switchAction: "Saved",
              clientInstruction: {
                url: switchTransaction.clientInstruction.url,
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (dateFormat_YY_MM_DD(selectedDate) > dateFormat_YY_MM_DD(Date.now())) {
      swal({
        ...
        text: `Do you want to record a Deposit that will be future dated to ${dateFormat_YY_MM_DD(
          selectedDate
        )}`,
        icon: "warning",
        ...
        if (edit) {
          swal({
            icon: "warning",
            text: `You are now recording a Deposit that will be future dated to ${dateFormat_YY_MM_DD(
              selectedDate
            )}`,
          });
          setSwitchTransaction({
      ...
      swal({
        ...
        text: `Do you want to record a Deposit that will be back dated to ${dateFormat_YY_MM_DD(
          selectedDate
        )}`,
        icon: "warning",
        ...
        if (edit) {
          swal({
            icon: "warning",
            text: `You are now recording a Deposit that will be back dated to ${dateFormat_YY_MM_DD(
              selectedDate
            )}`,
          });
          setSwitchTransaction({
  ...
  const onDelete = () => {
    ...
    store.switch.select(switchTransaction);
    showModalFromId(MODAL_NAMES.BACK_OFFICE.DELETE_SWITCH_CLOSE_OUT_ENTITIES_MODAL);
  };
  const onCancel = () => {
    setSwitchTransaction({ ...defaultSwitchTransaction });
    setSwitchTransactionTo({ ...defaultSwitchTransaction });
    setVisible(false);
    hideModalFromId(MODAL_NAMES.BACK_OFFICE.SWITCH_BETWEEN_ENTITIES_AMEND_MODAL_CLOSE_OUT);
    setSelectedClient(null);
    setSelectedClientTo(null);
    setSelectedClientAccountNumberTo({ label: "", value: "" })
    setSelectedClientAccountNumber({ label: "", value: "" })
      ...
      <div className="view-modal custom-modal-style uk-modal-dialog uk-modal-body uk-width-4-5">
        ...
        <div className="form-title">
          ...
          <h3 className="text-to-break" style={{ marginLeft: "2rem" }}>
            Amend Manual Switch Close Out Transaction
          </h3>
        </div>
        <hr />
        ...
        <div className="uk-margin-padding-top uk-text-right">
          ...
          <button type="button" className="btn btn-danger uk-margin-small-top" disabled={loading || saving} onClick={onDelete}>
            Delete
          </button>
        ...
        <Modal modalId={MODAL_NAMES.BACK_OFFICE.DELETE_SWITCH_CLOSE_OUT_ENTITIES_MODAL}>
          {showOnSubmitModal && <DeleteEntitySwitchCloseOutModal setVisible={setShowOnSubmitModal} />}
        </Modal>
      </div>
    </ErrorBoundary>
  );
...
export default AmendEntitySwitchModalCloseOut;
