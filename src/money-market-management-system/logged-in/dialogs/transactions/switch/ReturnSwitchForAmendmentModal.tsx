...
const ReturnSwitchForAmendmentModal = observer(({ setVisible }: IProps) => {
        ...
        if (fromAccount && toAccount) {
          if (futureDatingTransaction) {
            const _switchTransaction: ISwitchTransaction = {
              ...
              switchStatus: "Draft",
              switchAction: "Returned for Amendment",
              clientInstruction: {
  ...
  const onCancel = () => {
    ...
    hideModalFromId(MODAL_NAMES.BACK_OFFICE.RETURN_SWITCH_FOR_AMENDMENT_MODAL);
    setLoading(false);
    setVisible(false);
  };
  useEffect(() => {
    ...
    <ErrorBoundary>
      <div className="view-modal custom-modal-style uk-modal-dialog uk-modal-body uk-width-4-5">
        ...
        <div className="uk-margin-padding-top uk-text-right">
          ...
          <button className="btn btn-primary" disabled={loading} onClick={returnForAmendment}>Return for amendment</button>
        </div>
      </div>
    </ErrorBoundary>
  );
...
export default ReturnSwitchForAmendmentModal;
