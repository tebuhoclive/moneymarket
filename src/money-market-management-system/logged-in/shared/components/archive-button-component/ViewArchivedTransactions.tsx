...
const ViewArchivedTransactions = ({ data }: IProps) => {
    const [startDate, setStartDate] = useState<string>(new Date().toISOString().split('T')[0]);
    const [endDate, setEndDate] = useState<string>(new Date().toISOString().split('T')[0]);
    const [filteredTransactions, setFilteredTransactions] = useState<IDepositTransaction[]>([]);
    ...
    const formattedData = filteredTransactions.map((d) => {
        ...
        const $clientAccount = d.accountNumber;
        const $clientName = d.accountName;
        const $amount = d.amount;
        const $archivedBy = d.capturedBy;
                ...
                <table className="kit-table-bordered uk-margin-small-top">
                    ...
                    <tbody>
                        {filteredTransactions.map(d => (
                            <tr key={d.id}>
                                <td>{dateFormat_YY_MM_DD(d.transactionDate)}</td>
                                <td>{d.bankReference}</td>
                                <td>{d.accountNumber || "un-allocated"}</td>
                                <td>{d.accountName}</td>
                                <td>{d.amount}</td>
                                <td>{getUser(d.capturedBy ?? "", store)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
export default ViewArchivedTransactions;
