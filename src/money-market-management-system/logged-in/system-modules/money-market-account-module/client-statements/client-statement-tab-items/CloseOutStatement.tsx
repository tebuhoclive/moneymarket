...
interface IProps {
    closeOutDate: number;
    closeOutTransaction: IWithdrawalTransaction;
...
const CloseOutStatement = (props: IProps) => {

    const { api, store } = useAppContext();

    const { account, closeOutDate, closeOutTransaction, interest } = props;
                ...
                {!loading &&
                    <table className="uk-table uk-table-small kit-table">
                        ...
                        <tbody>
                            ...
                            {filteredStatementTransactions && filteredStatementTransactions.map((transaction, index) => (
                            ...
                            <tr>
                                <td>{dateFormat_YY_MM_DD(closeOutTransaction.valueDate)}</td>
                                <td>
                                    {`${(interest)}`}
                                </td>
                            ...
                            <tr>
                                ...
                                <td>{(account.balance + interest)}</td>
                                <td>{(closeOutTransaction.amount - closeOutTransaction.amount)}</td>
                                <td>{(account.clientRate || 0)}</td>
                                <td>{"-"}</td>
                                <td>{"-"}</td>
...
export default CloseOutStatement
