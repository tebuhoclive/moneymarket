...
import { calculateInterest, getFilteredStatementCloseOutTransactions, getFilteredStatementTransactions, getStatementTotalDays, getStatementTotalDistribution, getTotalDaysInMonth } from "../../../../../../shared/functions/transactions/Statement";
import { daysLogic, interestLogic } from "../../../../../../shared/functions/MyFunctions";
...
const BackDatedClientStatement = (props: IProps) => {

    const { api, store } = useAppContext();

    const [startDate, setStartDate] = useState<Date>(startOfMonth);
    const [endDate, setEndDate] = useState<Date>(endOfMonth);
    const [closeOutDate, setCloseOutDate] = useState<number>(Date.now());

    const { moneyMarketAccountId, generateAndSendPDF, exportAccountStatement, noButtons} = props;
    ...
    const filteredStatementTransactions = getFilteredStatementCloseOutTransactions(startDate, endDate, statementTransactionsAsJson, closeOutDate);
                ...
                <div className="uk-width-2-3 uk-flex">
                    ...
                    <div className="uk-margin-left">
                        ...
                        <input
                            ...
                            type="date"
                            id="endDate"
                            value={dateFormat_YY_MM_DD(closeOutDate)}
                            onChange={(e) => setCloseOutDate(e.target.valueAsNumber)}
                        />
                    </div>
                </div>
                            ...
                            {filteredStatementTransactions &&
                                filteredStatementTransactions.map((transaction, index) => (
                                    <tr className={transaction.blinded ? 'uk-text-danger' : ''} key={transaction.id}>
                                        ...
                                        <td>{daysLogic(transaction.days || 0)}</td>
                                        <td>{interestLogic((transaction.distribution || 0))}</td>
...
export default BackDatedClientStatement
