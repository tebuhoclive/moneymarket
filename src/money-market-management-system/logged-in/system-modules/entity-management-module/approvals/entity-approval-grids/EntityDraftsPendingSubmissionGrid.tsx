...
export const EntityDraftsPendingSubmissionGrid = observer(({ data }: IProps) => {
    ...
    const columns: GridColDef[] = [
     ...
     width:200,
            headerClassName: "grid",
            valueGetter: (params) => {
                return `${dateFormat_YY_MM_DD(params.row.timeDrafted)} ${getTimeFromTimestamp(params.row.timeDrafted)}`;
            },
        },
        {
            field: "status",
    ...
    return (
        <div className="grid">
            <h4 className="main-title-md">Pending Submission ({data.length})</h4>
            <Box sx={{ height: 300 }}>
                <DataGrid
                    rows={data}
                    columns={columns}
