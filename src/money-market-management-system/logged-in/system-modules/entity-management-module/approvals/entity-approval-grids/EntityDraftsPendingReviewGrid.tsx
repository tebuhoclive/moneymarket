...
export const EntityDraftsPendingReviewGrid = observer(({ data }: IProps) => {
    ...
    const columns: GridColDef[] = [
     ...
     width:200,
            headerClassName: "grid",
            valueGetter: (params) => {
                return `${dateFormat_YY_MM_DD(params.row.lastUpdated)} ${getTimeFromTimestamp(params.row.lastUpdated)}`;
            },
        },
        {
            field: "status",
    ...
    return (
        <div className="grid">
            <h4 className="main-title-md">Pending Review ({data.length})</h4>
            <Box sx={{ height: 300 }}>
                <DataGrid
                    rows={data}
                    columns={columns}
