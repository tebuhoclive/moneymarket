...
import SecondLevelWithdrawalTransactionQueueView from "./transaction-views/second-level-transaction-view/SecondLevelTransactionView";
...
export const WithdrawalSecondLevelApprovalGrid = observer(({ data, transactionFilter, loading }: IProp) => {
  ...
  const secondLevelQueue: IWithdrawalTransactionQueueData[] = data
    .sort((a, b) => {
  ...
  const filteredData = secondLevelQueue.filter((d) => {
      ...
      <DataGridToolbar
        rightControls={
          <button disabled={loadingSubmit || selectedTransactions.length === 0} className="btn btn-primary"
            onClick={approveSelectedForPaymentQueue}
          >
            {loadingSubmit ? <span data-uk-spinner={"ratio:.5"}></span> : `Submit (${selectedTransactions.length}) for Payment `}
          </button>
        }
      />
    );
  ...
  const approveSelectedForPaymentQueue = async () => {
  ...
  return (
    <div className="grid">
      <DataGrid
        ...
        rowHeight={50}
      />
      {showOnSubmitModal && <Modal modalId={MODAL_NAMES.BACK_OFFICE.WITHDRAWAL_TRANSACTION_FIRST_LEVEL_VIEW}>
        <SecondLevelWithdrawalTransactionQueueView />
      </Modal>}
